...
abstract public class MissileWeapon extends Weapon {
	...
	public int max() {
		if (Dungeon.hero.buff(ReflowBuff.class) != null) {
			return Math.max(0, max( buffedLvl() + (RingOfSharpshooting.levelDamageBonus(Dungeon.hero) + (Dungeon.hero.lvl /4) + (Dungeon.hero.STR /4))));
		}
	...
	public int throwPos(Hero user, int dst) {
		...
		if (!projecting && Random.Int(3) < user.pointsInTalent(Talent.SHARPSHOOTER)){
	...
	public int proc(Char attacker, Char defender, int damage) {
		if (attacker == Dungeon.hero && Random.Int(3) < Dungeon.hero.pointsInTalent(Talent.SHARPSHOOTER)){
			if (this instanceof Dart && ((Dart) this).crossbowHasEnchant(Dungeon.hero)){
				//do nothing
			} else {
				SpiritBow bow = Dungeon.hero.belongings.getItem(SpiritBow.class);
