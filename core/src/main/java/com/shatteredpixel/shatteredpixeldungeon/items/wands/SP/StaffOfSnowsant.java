...
public class StaffOfSnowsant extends Wand {
    private int min(int lvl) {
        return 1 + lvl;
    }

    private int max(int lvl) {
        return 7 + lvl * 5;
    }
    ...
    protected void fx(Ballistica bolt, Callback callback) {
        curUser.sprite.parent.add(new Chains(curUser.sprite.center(),
    ...
    private void chainEnemy(Ballistica chain, final Hero hero, final Char enemy) {
        ...
        final int oldPos = enemy.pos;
        enemy.pos = pulledPos;
        Dungeon.level.occupyCell(enemy);

        //push animation is processed by wandUsed()'s spendAndNext() since it's already in the actor queue
        Actor.add(new Pushing(enemy, oldPos, pulledPos, new Callback() {
            public void call() {
                Dungeon.observe();
                GameScene.updateFog();
            }
